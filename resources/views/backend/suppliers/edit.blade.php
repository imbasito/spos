    ...
    <div class="card shadow-sm border-0 border-radius-15 overflow-hidden">
      ...
      <div class="card-header bg-gradient-maroon py-3 d-flex align-items-center">
        <h5 class="card-title font-weight-bold text-white mb-0">
          <i class="fas fa-edit mr-2"></i> Edit Supplier — {{ $supplier->name }}
        </h5>
      ...
      <div class="card-body p-4">
        ...
        <form action="{{ route('backend.admin.suppliers.update', $supplier->id) }}" method="post">
          @method('PUT')
          @csrf
          <div class="row">
            <div class="mb-3 col-md-6">
              ...
              <input type="text" class="form-control @error('name') is-invalid @enderror"
                placeholder="Enter supplier name" name="name" value="{{ old('name', $supplier->name) }}" required>
            ...
            <div class="mb-3 col-md-6">
              ...
              <input type="text" class="form-control @error('phone') is-invalid @enderror"
                placeholder="Enter phone number" name="phone" value="{{ old('phone', $supplier->phone) }}" required>
              @error('phone')<div class="invalid-feedback">{{ $message }}</div>@enderror
            </div>
            <div class="mb-3 col-md-12">
              <label class="form-label font-weight-bold text-dark">Address</label>
              <input type="text" class="form-control @error('address') is-invalid @enderror"
                placeholder="Enter address (optional)" name="address" value="{{ old('address', $supplier->address) }}">
